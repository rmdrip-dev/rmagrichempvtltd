...
import { Menu, X, ShoppingCart, Leaf, Phone, Mail, MapPin } from 'lucide-react';
...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const isActive = (path: string) => location.pathname === path ? 'text-brand-500 font-semibold' : 'text-gray-600 hover:text-brand-500 transition-colors';
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <Link to="/" className="flex items-center space-x-2">
              ...
              <div>
                <span className="text-xl font-bold text-brand-900 block leading-none">RM</span>
                <span className="text-sm font-medium text-brand-600 tracking-wider">AGRICHEM</span>
              </div>
            </Link>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white border-t border-gray-100">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 flex flex-col">
              <Link to="/" onClick={() => setIsMenuOpen(false)} className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-brand-600 hover:bg-brand-50">Home</Link>
              <Link to="/about" onClick={() => setIsMenuOpen(false)} className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-brand-600 hover:bg-brand-50">About Us</Link>
              <Link to="/products" onClick={() => setIsMenuOpen(false)} className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-brand-600 hover:bg-brand-50">Products</Link>
              <Link to="/contact" onClick={() => setIsMenuOpen(false)} className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-brand-600 hover:bg-brand-50">Contact</Link>
            </div>
          </div>
        )}
      </nav>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div>
              ...
              <p className="text-gray-400 mb-6">
                Empowering farmers with world-class sustainable agricultural solutions. 
                Dedicated to maximizing yield and preserving soil health.
              </p>
            </div>
